...
//	}
	...
	// way-4
	@RequestMapping(value = "/todos.htm", method = RequestMethod.POST,produces= {"application/json","application/xml"})
	public @ResponseBody Todo postTodo3(@ModelAttribute Todo todo, BindingResult result) {
//		if (result.hasErrors()) {
//			return result.getAllErrors().stream().map(error -> error.getDefaultMessage())
//					.collect(Collectors.joining(",", "{", "}"));
//		}
